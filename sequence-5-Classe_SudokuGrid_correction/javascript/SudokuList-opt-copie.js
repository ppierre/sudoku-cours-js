...
//
// Optimisation : utilise un cache pour powerSet
// * Assumme que les éléments constituant la liste ne changent pas
// * Gain : Aprés la 1er itération 1s de moins par itération

  powerSet: function () {
    if (!$defined(this.cache_powerSet)) {
      this.cache_powerSet = this.data.powerSet().map(function (array) {
        return new SudokuList(array);
      });
    }
    return this.cache_powerSet;
  },
...
//
// Optimisation : utilise un cache pour isComplete
// * Assumme que l'on retire seulement des valeurs 
// ** donc une liste (** qui est compléte **) reste complète aprés un retrait
// * Gains : 1s pour les derniéres itérations (grand nombres de liste déjà traitées)

  isComplete : function () {
    if (!this.cache_isComplete) {
      this.cache_isComplete = this.data.length === this.combineAll().getLength();
    }
    return this.cache_isComplete;
  },
