...
// Retourne une chaine de caractère représentant le nom de la carte
  ...
  nom: function () {
    return RANGS[this.rang] + " de " + COULEURS[this.couleur];
  },
...
var Tas = new Class({
  initialize: function () {
    ...
    COULEURS.forEach(function (nom_couleur, couleur) {
      // pour chaques rangs
      RANGS.forEach(function (nom_rang, rang) {
        this.cartes.push(new Carte(rang, couleur));
      }, this);
    }, this);
...
// Donne un nombre déterminer de carte
  ...
  donnes_en: function (nbr) {
    ...
    if (nbr > this.cartes.length) {
      ...
      var a_donner = this.cartes.splice(-nbr, nbr);
      return a_donner;
    }
  },
...
// Montre la dernière cartes du tas
  ...
  montre_la_derniere: function () {
    ...
    var derniere = this.cartes[this.cartes.length - 1];
    derniere.rend_visible();
    return this;
  },
...
// Donne la derniére carte en la retirant du tableau
  ...
  donne_la_derniere: function () {
    ...
    if (this.cartes.length < 1) {
      ...
      var a_donner = this.cartes.pop();
      // Si il y a encore au moins une carte
      if (this.cartes.length > 0) {
        this.montre_la_derniere();
      }
...
// Retourne un booléen
  ...
  peut_prendre: function (carte) {
    ...
    return this.pile_couleurs[carte.couleur].length === carte.rang;
  },
...
// Retourne un booléen
  ...
  peut_prendre: function (nvl_carte) {
    ...
    if (nvl_carte.rang !== (derniere_carte.rang + 1)) {
      ...
      if ((derniere_carte.couleur === PIQUE) || (derniere_carte.couleur === TREFLE)) {
        // Si on va sur pique ou trèfle on doit avoir coeur ou carreau
        return (nvl_carte.couleur === COEUR) || (nvl_carte.couleur === CARREAU);
      } else {
        // Sin on a coeur ou carreau on doit avoir pique ou trèfle
        return (nvl_carte.couleur === PIQUE) || (nvl_carte.couleur === TREFLE);
      }
    }
  },
...
// Retourne {{{true}}} si il a réussi ; ({{{false}}} si non)
  ...
  de_tas_a_colonne: function (colonne) {
    // Demande si il peut (mais ne déplace pas)
    if (colonne.peut_prendre(this.tas.quelle_est_la_derniere())) {
      // On peut déplacer la derniére ccarte
      // on prend sur le tas
      var carte = this.tas.donne_la_derniere();
      // on la place sur la pile
